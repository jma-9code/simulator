...
/**
 * Generates an expandable ToolTip, which is a popup dispaying a short
 * description of the component (ToolTip) but also, when focused, allows for
 * displaying a longer help text
 * 
 * @author d.bauer
 * 
 */
public class ExpandableToolTip implements KeyListener, MouseListener,
		FocusListener, HyperlinkListener {
	JFrame popup; // PopUp
	...
	boolean helpActive = false; // switch indicating if the interactiv help
								// popUp is active
	...
	public void keyPressed(KeyEvent arg0) {
		if (arg0.getKeyCode() == KeyEvent.VK_F1) {
			...
			/*
			 * request Focus in editor so that it can be hidden when focus is
			 * lost
			 */
			h.requestFocus();
	...
	/**
	 * If 'd' was typed swap toolTip-popUp with help-popup and set helpActive to
	 * true to prevent that the mouseExited event causes the popup to go away.
	 */
	...
	/**
	 * Hide popUp if not 'd' was pressed before and the help popUp is now
	 * displayed
	 */
