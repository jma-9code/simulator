...
public class ChipStrategy implements IStrategy<ComponentIO> {
	 ...
	 * @param data
	 */
	private IResponse manageARPC_RQ(ComponentIO chip, Mediator m,
		...
		// head
		sb.append(ISO7816Tools
				.convertType2CodeMsg(MessageType.TRANSCATION_VAL_NOTIF));
		sb.append("007");

		// data
		sb.append(ISO7816Tools.createformatTLV(ISO7816Tools.FIELD_POSID, posID));
		sb.append(ISO7816Tools.createformatTLV(ISO7816Tools.FIELD_OPCODE,
				opcode));
		sb.append(ISO7816Tools.createformatTLV(ISO7816Tools.FIELD_AMOUNT,
				amount));
		if (apcode_tpe.equalsIgnoreCase(apcode_cb)) {
			sb.append(ISO7816Tools.createformatTLV(
					ISO7816Tools.FIELD_APPROVALCODE, apcode_cb));
			sb.append(ISO7816Tools.createformatTLV(
					ISO7816Tools.FIELD_RESPONSECODE, rpcode));
			...
			sb.append(ISO7816Tools.createformatTLV(ISO7816Tools.FIELD_DATETIME,
					ISO7816Tools
							.writeDATETIME(Calendar.getInstance().getTime())));
		} else {
	...
	private IResponse manageAUTH_RQ(ComponentIO chip, Mediator m,
		...
		String pan = chip.getProperties().get("pan");
		String protocol = chip.getProperties().get(
				ISO7816Tools.FIELD_POSID + "-" + posID);
		...
		// head
		sb.append(ISO7816Tools
				.convertType2CodeMsg(MessageType.CARDHOLDER_AUTH_RP));
		sb.append("007");
		// data
		sb.append(ISO7816Tools.createformatTLV(ISO7816Tools.FIELD_POSID, posID));
		sb.append(ISO7816Tools.createformatTLV(ISO7816Tools.FIELD_OPCODE,
		...
		if (ISO7816Tools.readAMOUNT(amount) > Integer.parseInt(chip
				.getProperties().get("ceil"))) {
			sb.append(ISO7816Tools.createformatTLV(
					ISO7816Tools.FIELD_CARDAGREEMENT, "0"));
		} else {
			sb.append(ISO7816Tools.createformatTLV(
					ISO7816Tools.FIELD_CARDAGREEMENT, "1"));
		}
		sb.append(ISO7816Tools.createformatTLV(
				ISO7816Tools.FIELD_PINVERIFICATION, "1"));
		sb.append(ISO7816Tools.createformatTLV(ISO7816Tools.FIELD_PAN, pan));
		sb.append(ISO7816Tools.createformatTLV(ISO7816Tools.FIELD_DATETIME,
	 ...
	 * @param data
	 */
	private IResponse manageSC_RQ(ComponentIO chip, Mediator m,
			HashMap<String, String> data) {
		String protocol_choice = null;
		List<String> protocols = Arrays.asList(chip.getProperties()
				.get("protocol").split(" "));
		String pref_protocol_tpe = data
				.get(ISO7816Tools.FIELD_PROTOCOLPREFERRED);
		String posID = data.get(ISO7816Tools.FIELD_POSID);
		...
		if (protocols.contains(pref_protocol_tpe)) {
			...
			List<String> protocols_tpe = Arrays.asList(data
					.get(ISO7816Tools.FIELD_PROTOCOLLIST));
			boolean possible = protocols.retainAll(protocols_tpe);
			if (possible) {
				// prend la premiere intersection
				protocol_choice = protocols.get(0);
			} else {
				log.debug("chip can't communicate with TPE protocols : "
						+ protocols_tpe);
				return DataResponse.build(m, "");
			}
		}
		...
		chip.getProperties().put(ISO7816Tools.FIELD_POSID + "_" + posID,
				protocol_choice);
		...
		// head
		sb.append(ISO7816Tools
				.convertType2CodeMsg(MessageType.SECURE_CHANNEL_RP));
		sb.append("003");
		// data
		sb.append(ISO7816Tools.createformatTLV(ISO7816Tools.FIELD_POSID, posID));
		sb.append(ISO7816Tools.createformatTLV(ISO7816Tools.FIELD_PROTOCOL,
				protocol_choice));
		sb.append(ISO7816Tools.createformatTLV(ISO7816Tools.FIELD_DATETIME,
				ISO7816Tools.writeDATETIME(Calendar.getInstance().getTime())));
